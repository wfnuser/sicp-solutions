      ...
      (cond ((null? vars)
            ...
            ((eq? var (car vars))
             (if (eq? (car vals) '*unassigned*)
                 (error "The value is unassigned " (car vars) (car vals))
                 (car vals)))
...
(define (make-let bindings settings rest)
  (append (append (list 'let bindings) settings) rest)
  ; (cons 'let (cons bindings (cons settings rest)))
)

(define (make-binding clause)
  (list (cadr clause) '*unassigned*) 
)

(define (make-set! clause)
  (list 'set! (cadr clause) (caddr clause))
)
...
(define (scan-out-defines body)
  (let* 
    (
      (defines (filter (lambda (x) (and (pair? x) (eq? (car x) 'define))) body))
      (rest (filter (lambda (x) (not (and (pair? x) (eq? (car x) 'define)))) body))
    )
    
    (make-let 
      (map make-binding defines) 
